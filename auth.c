...
void hashPassword(const char *plain, char *outHex) {
    char salted[256];
    snprintf(salted, sizeof(salted), "cms_salt_v2_%s", plain);
    SHA256_CTX ctx;
    sha256_init(&ctx);
...
static int userExists(const char *username) {
    ...
    char storedHash[128];
    while (fscanf(fp, "%31s %127s", storedUser, storedHash) == 2) {
...
int registerUser(void) {
    ...
    if (userExists(username)) {
        printf("Username '%s' is already registered! Please choose another or login.\n", username);
        return 0;
    }
    ...
    if (scanf("%63s", confirm) != 1) return 0;
...
int loginUser(void) {
    ...
    while (fscanf(fp, "%31s %127s", storedUser, storedHash) == 2) {
        if (strcmp(storedUser, username) == 0) {
            if (strcmp(storedHash, hash) == 0 || strcmp(storedHash, password) == 0) {
                authenticated = 1;
                strncpy(currentUser, storedUser, sizeof(currentUser) - 1);
...
const char* getCurrentUser(void) {
    return userLoggedIn ? currentUser : "Guest";
}
